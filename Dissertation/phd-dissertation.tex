...
\newcommand{\td}{\mathbf{t}}   % response-threshold value
...
One of the most elusive but important goals of swarm robotics is to reproduce the emergent collaborative behavior observed in natural swarming systems through the use of simple decision rules. Examples of collaborative processes in insect colonies such as foraging, scouting (finding shortest paths) for food, and colony defense involve some form of task allocation among individual agents. The robustness of task completion even after major environmental changes is also observed in natural swarm systems. Ants and bees are often unphased by the fact that the magnitude of a task---such as carrying a heavy piece of food---is unknown to every individual and manage to complete the task elegantly even without such critical knowledge. This robustness property is of paramount importance when recreating natural behavior in artificial systems and I believe the use of decentralized, agent based task allocation rules is closely related to this property. I therefore present an in-depth analysis of distributed response threshold based strategies for task allocation in multi-agent systems in this dissertation. I prove, using a well known result from the theory of global games, that under the constraints of imperfect knowledge of the environment and imperfect communication response threshold based task allocation leads to an equilibrium inducing strategy for the swarm system. The importance of this result is to provide a formal mathematical basis for the phenomenological approaches currently employed in the field of swarm robotics to mimic biological systems. This result therefore provides both, a hypothesis about the inner workings of a wide range of existing approaches with limited communication between agents in artificial swarm systems and also a formal explanation for threshold based task allocation in social insects. These game theory results lead to a novel continuous response threshold algorithm for multi-agent task allocation that generalizes fixed-group task allocation (stick-pulling experiment) and stochastic team size task allocation. This allows variable team sizes to form at task sites within tolerance limits thereby providing a trade-off between exploration and exploitation. Finally, the claims made by the theoretical proofs for response threshold based task allocation are backed up by physical experiments simulating a forest firefighting scenario using the \emph{Droplet} swarm robot platform. The experiments provide a basis of comparison between optimal centralized approaches where everything about the system is known to a central controller and distributed approaches for task allocation where each robot decides when to participate in a task based on its own noisy sensory input and imperfect knowledge of the system. I show that in many real world situations it is often impractical to rely on the assumption of perfect system information for controlling a swarm and that optimal centralized task allocation becomes comparable to a threshold-based policy under the influence of noise.
}
...
%- Apply global games theory and results to the domain for swarm intelligence (both swarm robotics and social insects)
%
%- Show that response thresholds lead to an equilibrium strategy.
%
%- Show that optimal task allocation becomes comparable to a threshold-based policy under the influence of noise. 
%
%- shown that threshold based task allocation using sigmoidal response threshold functions generalizes fixed-group task allocation ("stick pulling") and random task allocation, allowing to tune a trade-off between exploration and exploitation via the slope parameter (DARS paper)
...
\begin{table}[!ht]
\centering\begin{tabular}{|l|p{.65\columnwidth}|}
\hline
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\section{Experiments}
We use the Droplet swarm robot platform to perform experiments to study both the centralized and distributed approaches to TA mentioned in the previous section. Through the use of these experiments we show that a centralized approach is always preferred in situations where perfect information about the system is discernible but the optimality of the centralized approach quickly drops off as soon as this requirement is not met. In systems with imperfect information a distributed approach is often preferred when 
